...
{
	public static class am_map
	{
		public static r_local.vertex_t[] KeyPoints = new r_local.vertex_t[(int)DoomDef.keytype_t.NUMKEYS] {
	new r_local.vertex_t(),
	new r_local.vertex_t(),
	new r_local.vertex_t()
};

		public const int NUMALIAS = 3; // Number of antialiased lines.
...
public static bool    automapactive = false;
static int finit_width = DoomDef.SCREENWIDTH;
static int finit_height = DoomDef.SCREENHEIGHT-42;
...
static int m_w, m_h;
static int min_x, min_y; // based on level size
static int max_x, max_y; // based on level size
static int max_w, max_h; // max_x-min_x, max_y-min_y
static int min_w, min_h; // based on player size
...
static int scale_mtof = INITSCALEMTOF;
...
void AM_rotate(int *x, int *y, uint a)
{
  int tmpx;
...
void AM_drawLineCharacter(mline_t *lineguy, int lineguylines, int scale,
  uint angle, int color, int x, int y)
{
  int i;
  mline_t l;
