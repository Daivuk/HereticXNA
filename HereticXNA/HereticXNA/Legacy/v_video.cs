...
int dirtybox[4];
#endif
		public static m_misc.var_int usegamma = new m_misc.var_int();
	//	static public m_misc.var_bool ambient_enabled = new m_misc.var_bool();
	//	static public m_misc.var_bool use_deferred = new m_misc.var_bool();
	//	static public m_misc.var_bool postprocess_enabled = new m_misc.var_bool();
#if DOS
...
//---------------------------------------------------------------------------
		...
		public class PatchTexture
		{
			public DoomData.patch_t patch;
			public Texture2D texture;
		}

public static void BuildPatchTextureIfNot(w_wad.CacheInfo patch)
{
	if (patch == null) return;
	if (patch.cache == null)
	{
		// Load the texture
		DoomData.patch_t realPatch = new DoomData.patch_t();
		BinaryReader bs = new BinaryReader(new MemoryStream(patch.data));
		...
		Texture2D xnaTexture = new Texture2D(Game1.instance.GraphicsDevice, realPatch.width + 2, realPatch.height + 2);
		Color[] xnaTextureData = new Color[(realPatch.width + 2) * (realPatch.height + 2)];
		for (int k = 0; k < realPatch.width; ++k)
		{
			DoomData.post_t patchcol = new DoomData.post_t();
			patchcol.topdelta = patch.data[realPatch.columnofs[k]];
			patchcol.length = patch.data[realPatch.columnofs[k] + 1];

			r_data.R_DrawColumnInCache(patch.data, realPatch.columnofs[k], patchcol, k, xnaTextureData, 0, realPatch.width + 2, realPatch.height + 2, 1, 1);
		}
		xnaTexture.SetData(xnaTextureData);
		Game1.instance.patchTextures[patch.lumpInfo.name] = xnaTexture;
		Game1.instance.allTextures.Add(xnaTexture);
		patch.cache = new PatchTexture { patch = realPatch, texture = xnaTexture };
	}
}

public static void BuildRawTextureIfNot(w_wad.CacheInfo patch)
{
	if (patch.cache == null)
	{
		Texture2D xnaTexture = new Texture2D(Game1.instance.GraphicsDevice, 
			320, 
			200);
		Color[] xnaTextureData = new Color[320 * 200];
		w_wad.CacheInfo palette = w_wad.W_CacheLumpName("PLAYPAL", DoomDef.PU_CACHE);
		for (int k = 0; k < 320 * 200; ++k)
		{
			int component = (int)patch.data[k];
			xnaTextureData[k] = new Color(
		...
		Game1.instance.patchTextures[patch.lumpInfo.name] = xnaTexture;
		Game1.instance.allTextures.Add(xnaTexture);
		patch.cache = xnaTexture;
	}
}
	
public static void V_DrawPatch(int x, int y, w_wad.CacheInfo patch)
{
	if (patch == null) return;

	float ratiox = (float)Game1.instance.GraphicsDevice.Viewport.Width / 320.0f;
	float ratioy = (float)Game1.instance.GraphicsDevice.Viewport.Height / 200.0f;

	BuildPatchTextureIfNot(patch);
	...
	PatchTexture textureCache = patch.cache as PatchTexture;
	...
	x = (int)((float)(x - textureCache.patch.leftoffset - 1) * ratiox);
	y = (int)((float)(y - textureCache.patch.topoffset - 1) * ratioy);
	Game1.instance.spriteBatch.Draw(textureCache.texture,
		new Rectangle(x, y,
			(int)(Math.Ceiling((float)textureCache.texture.Width * ratiox)),
			(int)(Math.Ceiling((float)textureCache.texture.Height * ratioy))), 
		Color.White);
	Game1.instance.spriteBatch.End();
}
...
public static void V_DrawShadowedPatch(int x, int y, w_wad.CacheInfo patch)
{
	if (patch == null) return;
...
public static void V_DrawRawScreen(w_wad.CacheInfo patch)
{
	BuildRawTextureIfNot(patch);

	Game1.instance.spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.NonPremultiplied, SamplerState.LinearClamp, DepthStencilState.None, RasterizerState.CullNone);
	Texture2D textureCache = patch.cache as Texture2D;
	Game1.instance.spriteBatch.Draw(textureCache,
		new Rectangle(0, 0, Game1.instance.GraphicsDevice.Viewport.Width,
			Game1.instance.GraphicsDevice.Viewport.Height),
			// [dsl] I added those checks to make the menu more visible on top of the background
			(mn_menu.MenuActive && mn_menu.InfoType == 0) ? new Color(.25f, .25f, .25f) : Color.White);
	Game1.instance.spriteBatch.End();
}
